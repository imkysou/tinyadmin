...
function sendTellraw(player, component) {
    const snbt = JSON.stringify(component);
    plugin_executeCommand(`tellraw ${player} ${snbt}`);
}

function sendSuccess(player, msg) {
    sendTellraw(player, {
        text: `✔ [TinyAdmin] ${msg}`,
        color: 'green'
    });
...
function sendInfo(player, msg) {
    sendTellraw(player, {
        text: `ℹ [TinyAdmin] ${msg}`,
        color: 'yellow'
    });
}

function sendWarn(player, msg) {
    sendTellraw(player, {
        text: `⚠ [TinyAdmin] ${msg}`,
        color: 'gold'
    });
...
// 构建一个可点击的建议命令组件
function clickableCmd(cmd, label = cmd) {
    return {
        text: label,
        color: 'aqua',
...
function registerCommands() {
    ...
    plugin_registerCommand(PREFIX + 'ta', (player) => {
        ...
        const cmds = [
            `${PREFIX}ta creategroup <name>`,
            `${PREFIX}ta removegroup <group>`,
            `${PREFIX}ta group <组名> permission <节点> true|false`,
            `${PREFIX}ta user <玩家> permission <节点> true|false`,
            `${PREFIX}ta user <玩家> setgroup <优先级> <组名>`,
            `${PREFIX}ta removegroupmember <user> <group>`,
            `${PREFIX}ta listgroups`,
            `${PREFIX}ta listgroupmembers <group>`,
            `${PREFIX}ta whichgroup <user>`
        ];
        const component = {
            text: '===== TinyAdmin 帮助 =====',
            color: 'gold',
        ...
        cmds.forEach((cmd, index) => {
            component.extra.push(clickableCmd(cmd));
            if (index < cmds.length - 1) component.extra.push({ text: '\n' });
        });
        sendTellraw(player, component);
    });
    ...
    plugin_registerCommand(PREFIX + 'ta creategroup <name>', (player, name) => {
        ...
        db.get('SELECT name FROM groups WHERE name = ?', [name], (err, row) => {
            if (row) {
                sendError(player, `组 ${name} 已存在，无法创建`);
                return;
            }
            db.run('INSERT INTO groups (name) VALUES (?)', [name], (err) => {
                if (err) {
                    sendError(player, `创建组失败: ${err.message}`);
                    return;
                }
                sendSuccess(player, `组 ${name} 创建成功`);
            });
        });
    });
    ...
    plugin_registerCommand(PREFIX + 'ta group <groupName> permission <permission> <value>', (player, groupName, permission, value) => {
        ...
        db.get('SELECT permissions FROM groups WHERE name = ?', [groupName], (err, row) => {
            if (err || !row) {
                sendError(player, `组 ${groupName} 不存在`);
                return;
            }
            let perms = {};
            ...
            db.run('UPDATE groups SET permissions = ? WHERE name = ?', [JSON.stringify(perms), groupName], (err) => {
                if (err) {
                    sendError(player, `更新组权限失败: ${err.message}`);
                    return;
                }
                sendSuccess(player, `已为组 ${groupName} 设置权限 ${permission}: ${boolValue}`);
                reloadGlobalPermissions();
            });
        });
    });
    ...
    plugin_registerCommand(PREFIX + 'ta user <userName> permission <permission> <value>', (player, userName, permission, value) => {
        ...
        db.run('INSERT OR REPLACE INTO user_permissions (user_name, permission_node, value) VALUES (?, ?, ?)', [userName, permission, boolValue ? 1 : 0], (err) => {
            if (err) {
                sendError(player, `设置用户权限失败: ${err.message}`);
                return;
            }
            sendSuccess(player, `已为用户 ${userName} 设置权限 ${permission}: ${boolValue}`);
            reloadGlobalPermissions();
        });
    });
    ...
    plugin_registerCommand(PREFIX + 'ta user <userName> setgroup <priority> <groupName>', (player, userName, priorityStr, groupName) => {
        ...
        db.get('SELECT name FROM groups WHERE name = ?', [groupName], (err, row) => {
            ...
            db.run('INSERT OR REPLACE INTO user_groups (user_name, group_name, priority) VALUES (?, ?, ?)', [userName, groupName, priority], (err) => {
                if (err) {
                    sendError(player, `加入组失败: ${err.message}`);
                    return;
                }
                sendSuccess(player, `已将用户 ${userName} 加入组 ${groupName}（优先级 ${priority}）`);
                reloadGlobalPermissions();
            });
        });
    });
    ...
    plugin_registerCommand(PREFIX + 'ta removegroupmember <user> <group>', (player, user, group) => {
        ...
        db.run('DELETE FROM user_groups WHERE user_name = ? AND group_name = ?', [user, group], function(err) {
            if (err) {
                sendError(player, `移除失败: ${err.message}`);
                return;
            }
            if (this.changes === 0) {
                sendError(player, `玩家 ${user} 不在组 ${group} 中`);
                return;
            }
            db.get('SELECT COUNT(*) as cnt FROM user_groups WHERE user_name = ?', [user], (err, row) => {
                if (err) {
                    sendInfo(player, `已将 ${user} 从组 ${group} 移除`);
                    reloadGlobalPermissions();
                    return;
                }
                if (row.cnt === 0) {
                    db.run('INSERT INTO user_groups (user_name, group_name, priority) VALUES (?, ?, ?)', [user, 'default', 0], (err) => {
                        ...
                        sendSuccess(player, `已将 ${user} 从组 ${group} 移除，并自动加入 default 组`);
                        reloadGlobalPermissions();
                    });
                } else {
                    sendSuccess(player, `已将 ${user} 从组 ${group} 移除`);
                    reloadGlobalPermissions();
                }
            });
        });
    ...
    plugin_registerCommand(PREFIX + 'ta listgroups', (player) => {
        ...
        db.all('SELECT name FROM groups', (err, rows) => {
            if (err) {
                sendError(player, `查询失败: ${err.message}`);
                return;
            }
            if (rows.length === 0) {
                sendInfo(player, '当前没有任何组');
                return;
            }
            const component = {
                text: '所有组: ',
            ...
            rows.forEach((r, index) => {
                db.get('SELECT COUNT(*) as cnt FROM user_groups WHERE group_name = ?', [r.name], (err, cntRow) => {
                    const memberCount = err ? '?' : cntRow.cnt;
                    const groupComp = {
                        text: r.name,
                        color: 'aqua',
                        hoverEvent: {
                            action: 'show_text',
                            value: { text: `成员数: ${memberCount}\n点击查看成员`, color: 'gray' }
                        },
                        clickEvent: {
                            action: 'suggest_command',
                            value: `${PREFIX}ta listgroupmembers ${r.name}`
                        }
                    };
                    component.extra.push(groupComp);
                    if (index < rows.length - 1) component.extra.push({ text: ', ', color: 'white' });
            ...
            if (rows.length === 0) {
                sendTellraw(player, { text: '所有组: (无)', color: 'yellow' });
            }
        });
    });
        ...
        db.all('SELECT user_name, priority FROM user_groups WHERE group_name = ? ORDER BY priority DESC', [group], (err, rows) => {
            ...
            rows.forEach((r, index) => {
                component.extra.push({
                    text: `${r.user_name}(${r.priority})`,
                    color: 'green',
                    hoverEvent: {
                        action: 'show_text',
                        value: { text: `玩家: ${r.user_name}\n优先级: ${r.priority}`, color: 'gray' }
                    }
                });
                if (index < rows.length - 1) component.extra.push({ text: ', ', color: 'white' });
            });
    ...
    plugin_registerCommand(PREFIX + 'ta whichgroup <user>', (player, user) => {
        ...
        db.all('SELECT group_name, priority FROM user_groups WHERE user_name = ? ORDER BY priority DESC', [user], (err, rows) => {
            ...
            if (rows.length === 0) {
                sendInfo(player, `玩家 ${user} 当前无组记录（视为 default）`);
                return;
            }
            const component = {
                text: `玩家 ${user} 的组: `,
            ...
            rows.forEach((r, index) => {
                component.extra.push({
                    text: `${r.group_name}(${r.priority})`,
                    color: 'green',
                    hoverEvent: {
                        action: 'show_text',
                        value: { text: `组: ${r.group_name}\n优先级: ${r.priority}`, color: 'gray' }
                    }
                });
                if (index < rows.length - 1) component.extra.push({ text: ', ', color: 'white' });
            });
...
plugin_log('INFO', 'TinyAdmin 插件已加载！');